...
udp-cicd authenticates to the Fabric API using a service principal or a managed identity. No interactive browser login is needed.
...
udp-cicd uses `DefaultAzureCredential` from the Azure Identity SDK, which picks up these variables automatically.
